...
\StringTok{        valuation of a company relative to its book value"}\NormalTok{,}
        \StringTok{"Enterprise value-to-revenue"}\NormalTok{,  }\StringTok{"rev"}\NormalTok{, }\StringTok{"Also refers as the EV/R, it measures }
\StringTok{        the value of a stock that compares a company's enterprise value to its revenue"}\NormalTok{, }
        \StringTok{"EV/EBITDA"}\NormalTok{, }\StringTok{"ebitda"}\NormalTok{, }\StringTok{"Enterprise value to earnings before interest, taxed, }
\StringTok{        depreciation and amortization ratio compares the value of a company, debt }
\StringTok{        included to the company's cash earnings less non-cash expenses"}\NormalTok{, }
        \StringTok{"Total ESG risk score"}\NormalTok{, }\StringTok{"tot_risk"}\NormalTok{, }\StringTok{"The overall rating scores based on }
\StringTok{        the Morningstar Sustainability Rating systems"}\NormalTok{, }
        \StringTok{"Environmental Risk Score"}\NormalTok{, }\StringTok{"envir_risk"}\NormalTok{, }\StringTok{"Evaluation scores of the }
\StringTok{        portfolios performance when they meet the environmental challenges"}\NormalTok{, }
        \StringTok{"Social Risk Score"}\NormalTok{, }\StringTok{"social_risk"}\NormalTok{, }\StringTok{"Evaluation scores of the portfolios }
\StringTok{        performance when they meet the social challenges"}\NormalTok{, }
        \StringTok{"Governance Risk Score"}\NormalTok{, }\StringTok{"gover_risk"}\NormalTok{,}\StringTok{"Evaluation scores of the portfolios }
\StringTok{        performance when they meet the governance challenges"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
...
\NormalTok{df <-}\StringTok{ }\NormalTok{issuer[}\OperatorTok{!}\NormalTok{(issuer}\OperatorTok{$}\NormalTok{Symbol }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"MSFT"}\NormalTok{, }\StringTok{"AAPL"}\NormalTok{, }\StringTok{"TSLA"}\NormalTok{, }\StringTok{"ILMN"}\NormalTok{, }\StringTok{"DIS"}\NormalTok{, }\StringTok{"VZ"}\NormalTok{, }
                                    \StringTok{"KO"}\NormalTok{, }\StringTok{"MMM"}\NormalTok{,}\StringTok{"CVX"}\NormalTok{, }\StringTok{"PCAR"}\NormalTok{, }\StringTok{"CAT"}\NormalTok{, }\StringTok{"XOM"}\NormalTok{, }
                                    \StringTok{"V"}\NormalTok{, }\StringTok{"INTU"}\NormalTok{, }\StringTok{"NKE"}\NormalTok{)),]}
\CommentTok{#view(df)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{# filter out the outliers}
\NormalTok{risk <-}\StringTok{ }\KeywordTok{subset}\NormalTok{(stocks[(stocks}\OperatorTok{$}\NormalTok{Symbol }\OperatorTok{%in%}\StringTok{ }\NormalTok{df}\OperatorTok{$}\NormalTok{Symbol),], }
               \DataTypeTok{select =} \KeywordTok{c}\NormalTok{(}\StringTok{"Symbol"}\NormalTok{, }
                          \StringTok{"tot_risk"}\NormalTok{, }
                          \StringTok{"envir_risk"}\NormalTok{, }
                          \StringTok{"social_risk"}\NormalTok{, }
                          \StringTok{"gover_risk"}\NormalTok{))}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# First check whether total esg risk score = }
\CommentTok{# environmental risk + social risk + governance risk}
\NormalTok{d <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(}\KeywordTok{cbind}\NormalTok{(risk}\OperatorTok{$}\NormalTok{envir_risk, }
\NormalTok{                 risk}\OperatorTok{$}\NormalTok{social_risk, }
\NormalTok{                 risk}\OperatorTok{$}\NormalTok{gover_risk), }
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Second check whether total esg risk score = }
\CommentTok{# environmental risk + social risk + governance risk}
\NormalTok{risk_tesg <-}\StringTok{ }\NormalTok{risk[risk}\OperatorTok{$}\NormalTok{tot_risk }\OperatorTok{==}\StringTok{ }\NormalTok{d,]}
\NormalTok{d2 <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(}\KeywordTok{cbind}\NormalTok{(risk_tesg}\OperatorTok{$}\NormalTok{envir_risk, }
\NormalTok{                  risk_tesg}\OperatorTok{$}\NormalTok{social_risk, }
\NormalTok{                  risk_tesg}\OperatorTok{$}\NormalTok{gover_risk), }
            \DecValTok{1}\NormalTok{, sum)}
...
\NormalTok{memb_one <-}\StringTok{ }\NormalTok{dat[dat}\OperatorTok{$}\NormalTok{Symbol }\OperatorTok{%in%}\StringTok{ }
\StringTok{                  }\KeywordTok{rownames}\NormalTok{(}\KeywordTok{as.data.frame}\NormalTok{(memb_three_w) }\OperatorTok{%>%}\StringTok{ }
\StringTok{                             }\KeywordTok{filter}\NormalTok{(memb_three_w }\OperatorTok{==}\StringTok{ }\DecValTok{2}\NormalTok{)),]}\OperatorTok{$}\NormalTok{Name}
...
\NormalTok{memb_two <-}\StringTok{ }\NormalTok{dat[dat}\OperatorTok{$}\NormalTok{Symbol }\OperatorTok{%in%}\StringTok{  }
\StringTok{                  }\KeywordTok{rownames}\NormalTok{(}\KeywordTok{as.data.frame}\NormalTok{(memb_three_w) }\OperatorTok{%>%}\StringTok{ }
\StringTok{                             }\KeywordTok{filter}\NormalTok{(memb_three_w }\OperatorTok{==}\StringTok{ }\DecValTok{3}\NormalTok{)),]}\OperatorTok{$}\NormalTok{Name}

\NormalTok{memb_two }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kable}\NormalTok{(}\DataTypeTok{col.names =} \KeywordTok{c}\NormalTok{(}\StringTok{"stock"}\NormalTok{), }
        \DataTypeTok{caption =} \StringTok{"The stocks of the second cluster"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{bootstrap_options =} \KeywordTok{c}\NormalTok{(}\StringTok{"bordered"}\NormalTok{, }\StringTok{"hover"}\NormalTok{, }\StringTok{"striped"}\NormalTok{)) }
\end{Highlighting}
\end{Shaded}
...
\NormalTok{memb_three <-}\StringTok{ }\NormalTok{dat[dat}\OperatorTok{$}\NormalTok{Symbol }\OperatorTok{%in%}\StringTok{ }
\StringTok{                    }\KeywordTok{rownames}\NormalTok{(}\KeywordTok{as.data.frame}\NormalTok{(memb_three_w) }\OperatorTok{%>%}\StringTok{ }
\StringTok{                               }\KeywordTok{filter}\NormalTok{(memb_three_w }\OperatorTok{==}\StringTok{ }\DecValTok{1}\NormalTok{)),]}\OperatorTok{$}\NormalTok{Name}

\NormalTok{memb_three }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kable}\NormalTok{(}\DataTypeTok{col.names =} \KeywordTok{c}\NormalTok{(}\StringTok{"stock"}\NormalTok{), }
        \DataTypeTok{caption =} \StringTok{"The stocks of the third cluster"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{bootstrap_options =} \KeywordTok{c}\NormalTok{(}\StringTok{"bordered"}\NormalTok{, }\StringTok{"hover"}\NormalTok{, }\StringTok{"striped"}\NormalTok{)) }
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{rownames}\NormalTok{(al_method) <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"complete linkage method"}\NormalTok{, }
                         \StringTok{"average linkage method"}\NormalTok{, }
                         \StringTok{"centroid method"}\NormalTok{)}
