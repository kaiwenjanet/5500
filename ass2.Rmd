...
```{r}
stocks %>% filter(Enterprise.value.EBITDA<10) %>% ggplot(aes(Enterprise.value.EBITDA, Enterprise.value.EBITDA)) + geom_boxplot() + facet_wrap(~Industry)
```
...
biplot(pca_value, expand=10,xlim=c(-0.2,0.6),ylim=c(-0.02,0.1))
```
